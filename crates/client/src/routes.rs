pub const ROUTE_LOADING: &str = "/loading";
pub const ROUTE_SENTRY_REPORTS: &str = "/sentry-reports";
pub const ROUTE_IDENTITY_SETUP: &str = "/identity/setup";
pub const ROUTE_SEED_PHRASE: &str = "/identity/seed";
pub const ROUTE_IMPORT_PRIVATE_KEY: &str = "/identity/import";
pub const ROUTE_IMPORT_LEGACY_CONFIG: &str = "/identity/import-legacy-config";
pub const ROUTE_RESTORE_IDENTITY: &str = "/identity/restore";
pub const ROUTE_CHOOSE_SERVER: &str = "/servers";
pub const ROUTE_CONNECT_SERVER: &str = "/servers/connect";
pub const ROUTE_LOBBY: &str = "/lobby";
pub const ROUTE_SERVER_SETTINGS: &str = "/server/settings";
pub const ROUTE_SERVER_SETTINGS_CHANNELS: &str = "/server/settings/channels";
pub const ROUTE_SERVER_SETTINGS_MEMBERS: &str = "/server/settings/members";
pub const ROUTE_SERVER_SETTINGS_ROLES: &str = "/server/settings/roles";
pub const ROUTE_SETTINGS: &str = "/settings";
pub const ROUTE_SETTINGS_IDENTITY: &str = "/settings/identity";
pub const ROUTE_SETTINGS_SERVERS: &str = "/settings/servers";
pub const ROUTE_SETTINGS_AUDIO: &str = "/settings/audio";
pub const ROUTE_SETTINGS_NOTIFICATIONS: &str = "/settings/notifications";
pub const ROUTE_SETTINGS_STREAM: &str = "/settings/stream";
