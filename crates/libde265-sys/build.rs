...
fn compile_and_add_libde265_static_lib(root: &Path, libname: &str, encoder: bool) {
  ...
  let target_is_x86 = target.starts_with("i686-")
  ...
  let files = glob_import(&libde265_src, ".cc", &exclude);
  ...
  if target_os == "macos" {
    cc_build.define("HAVE_ALLOCA_H", "1").define("HAVE_POSIX_MEMALIGN", "1");
  } else {
    cc_build.define("HAVE_MALLOC_H", "1");
  }
...
fn build_libde265_from_sources() -> anyhow::Result<()> {
  ...
  if !lib_path.exists() {
    ...
    if !archive_root_dir.exists() {
      let bundled_archive = PathBuf::from(env::var("CARGO_MANIFEST_DIR")?)
        .join("vendor")
        .join(LIBDE265_FILE_NAME);
      if bundled_archive.exists() {
        fs::copy(bundled_archive, &archive_file)?;
      } else {
