use super::{
    BinaryReader, BinaryWriter, DecodeError, DecodeResult, UserId, VideoCodecId,
    VIDEO_FLAG_KEYFRAME,
};
...
impl ForwardedVoicePacket {
    pub fn decode(bytes: &[u8]) -> DecodeResult<Self> {
        ...
        Ok(Self { sender_id, sequence, opus })
    }
}
...
impl VideoControl {
    ...
    pub fn decode_datagram(bytes: &[u8]) -> DecodeResult<Self> {
        ...
        Ok(match subtype {
            ...
            _ => Self::Unknown { subtype, payload: r.read_remaining().to_vec() },
        })
    }
}
...
mod tests {
    ...
    fn video_control_pli_round_trips() {
        ...
        assert_eq!(encoded, vec![PacketType::VideoControl as u8, VIDEO_CTL_PLI, 42, 0, 0, 0]);
