...
pub enum C2S {
    ...
    ScreenShareStop,
    ScreenShareView { target_user_id: UserId },
    ScreenShareUpdate(ScreenShareMetadata),
    AdminCreateChannel { name: String, max_users: u32 },
    AdminDeleteChannel { channel_id: ChannelId },
    AdminSetRole { target_user_id: UserId, role: Role },
    AdminKickUser { target_user_id: UserId },
    AdminRenameChannel { channel_id: ChannelId, new_name: String },
    ChatSend {
        channel_id: ChannelId,
        text: String,
        attachments: Vec<ChatSendAttachment>,
    },
    ChatHistoryReq { channel_id: ChannelId, before_id: u64, limit: u16 },
    ChatPin { message_id: u64 },
    ChatUnpin { message_id: u64 },
    ChatDelete { message_id: u64 },
    ChatFileUploadReq { message_id: u64, file_index: u8, file_size: u64 },
    ChatFileDownloadReq { file_id: u64 },
    ChatSearch { channel_id: ChannelId, query: String, before_id: u64, limit: u16 },
    ChatPinnedReq { channel_id: ChannelId },
    AdminCreateTextChannel { name: String },
    AdminDeleteTextChannel { channel_id: ChannelId },
}
...
impl C2S {
    pub fn encode(&self) -> DecodeResult<ControlFrame> {
        ...
        let (ty, payload) = match self {
            ...
            Self::ChatSend { channel_id, text, attachments } => {
                let mut w = BinaryWriter::new();
                w.write_u32(*channel_id);
                w.write_string(text)?;
            ...
            Self::ChatHistoryReq { channel_id, before_id, limit } => {
                let mut w = BinaryWriter::new();
                w.write_u32(*channel_id);
            ...
            Self::ChatFileUploadReq { message_id, file_index, file_size } => {
                let mut w = BinaryWriter::new();
                w.write_u64(*message_id);
            ...
            Self::ChatSearch { channel_id, query, before_id, limit } => {
                let mut w = BinaryWriter::new();
                w.write_u32(*channel_id);
