use super::{
    BinaryReader, BinaryWriter, ChannelId, ChannelKeyBytes, DecodeError, DecodeResult, PublicKey,
    Role, SessionToken, Signature, UserId, VideoCodecId,
};
...
impl AuthResponse {
    pub fn decode_payload(bytes: &[u8]) -> DecodeResult<Self> {
        ...
        r.finish()?;
        Ok(Self { user_id, session_token, role, server_name })
    }
}
...
impl UserJoinedChannel {
    pub fn decode_payload(bytes: &[u8]) -> DecodeResult<Self> {
        ...
        r.finish()?;
        Ok(Self { user_id, username, channel_id, role })
    }
}
...
impl UserVoiceState {
    pub fn decode_payload(bytes: &[u8]) -> DecodeResult<Self> {
        ...
        r.finish()?;
        Ok(Self { user_id, muted, deafened })
    }
}
...
impl ChatFileUploadResponse {
    pub fn decode_payload(bytes: &[u8]) -> DecodeResult<Self> {
        ...
        r.finish()?;
        Ok(Self { message_id, file_index, accepted, reason })
    }
}
...
impl ChatHistoryResponse {
    pub fn decode_payload(bytes: &[u8]) -> DecodeResult<Self> {
        ...
        r.finish()?;
        Ok(Self { channel_id, has_more, messages })
    }
}
