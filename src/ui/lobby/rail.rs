...
type JoinChannelAction = lurq::app::ctx::FutureAction<ChannelId, (), String>;
...
impl Component for LobbyRail {
  ...
  fn render(&self, ctx: &mut Ctx) -> impl Into<Element> {
    ...
    let join_channel = session.clone().map(|session| join_channel_action(ctx, session));
...
fn join_channel_action(ctx: &mut Ctx, session: ServerSession) -> JoinChannelAction {
...
fn voice_control_action(ctx: &mut Ctx, session: ServerSession) -> VoiceControlAction {
  ctx.future_action(move |control| {
    ...
    async move {
      ...
      match control {
        VoiceControl::ToggleMute => muted = !muted,
  ...
  Row::new()
    ...
    .child(
      Column::new()
        ...
        .child(rail_channels(ctx, lobby, join_channel))
...
fn rail_header(ctx: &mut Ctx, info: &ConnectedServerInfo, lobby: &LobbyState) -> Element {
  ...
  let user_label = local_user_name(lobby, info).unwrap_or_else(|| format!("user #{}", info.user_id));
...
fn rail_channels(ctx: &mut Ctx, lobby: &LobbyState, join_channel: Option<&JoinChannelAction>) -> Element {
  Column::new()
    .width(Dimension::Pct(100.0))
    .flex(1.0)
    .spacing(18.0)
    ...
    .child(text_channels_section(ctx))
    .child(voice_channels_section(ctx, lobby, join_channel))
    .into()
}

fn text_channels_section(ctx: &mut Ctx) -> Element {
  Column::new()
    .width(Dimension::Pct(100.0))
    .spacing(theme::SpacingSize::Xs)
    .child(section_head(
...
fn voice_channels_section(ctx: &mut Ctx, lobby: &LobbyState, join_channel: Option<&JoinChannelAction>) -> Element {
  ...
  if lobby.channels.is_empty() {
    ...
    for channel in &lobby.channels {
      body = body.child(channel_row(ctx, channel, lobby.selected_channel_id, join_channel));
    }
  }
...
fn section_head(ctx: &mut Ctx, left_icon: &'static str, label: &str, right_icon: Option<&'static str>) -> Element {
  let mut row = Row::new()
    .width(Dimension::Pct(100.0))
    .align_items(Alignment::Center)
...
fn connection_status(ctx: &mut Ctx, lobby: &LobbyState) -> Element {
  ...
  let icon_color = if connected_to_voice {
...
fn local_user_row(ctx: &mut Ctx, info: &ConnectedServerInfo, lobby: &LobbyState) -> Element {
  ...
  let username = local_user_name(lobby, info).unwrap_or_else(|| format!("user #{}", info.user_id));
  let settings_button = if let Some(navigator) = navigator {
    icon_button(ctx, "settings", false).on_click(move |_| navigator.push(ROUTE_SETTINGS))
  } else {
    icon_button(ctx, "settings", false)
  };
...
fn control_row(
  ...
  let (muted, deafened) = local_voice_state(lobby, info);
  ...
  row = row.child(control_button(ctx, "monitor-up", false, None, None));

  let navigator = ctx.navigator();
  if let Some(navigator) = navigator {
    row = row.child(icon_button(ctx, "settings", false).on_click(move |_| navigator.push(ROUTE_SETTINGS)));
  } else {
    row = row.child(icon_button(ctx, "settings", false));
  }
...
fn control_button(
  ...
  let mut button = icon_button(ctx, icon, active);
...
fn icon_button(ctx: &mut Ctx, icon: &'static str, active: bool) -> Row {
  Row::new()
    .width(Dimension::Pct(100.0))
    .height(38.0)
    .flex(1.0)
