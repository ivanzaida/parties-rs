#include "windows_screen_capture.h"
...
bool ScreenCapture::init() {
    ...
    if (FAILED(hr)) {
        native_log_error("Failed to create D3D11 device ({:#010x})", static_cast<unsigned>(hr));
        return false;
    }
    ...
    try {
        ...
        native_log_error("WinRT device creation failed: {:#010x}",
    ...
    try {
        ...
        impl_->closed_token = impl_->item.Closed(
            [this](GraphicsCaptureItem const&, winrt::Windows::Foundation::IInspectable const&) {
                native_log_warn("Capture item closed");
                capturing_ = false;
        ...
        native_log_error("Failed to start capture: {:#010x}",
                     static_cast<unsigned>(e.code()));
        return false;
    }
}
