...
bool NvencEncoder::init(ID3D11Device* device, uint32_t width, uint32_t height,
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("OpenEncodeSessionEx failed: {}", (int)status);
        return false;
    }
    ...
    status = funcs_.nvEncGetEncodePresetConfigEx(
        ...
        &preset_config);
    if (status != NV_ENC_SUCCESS) {
        native_log_error("GetEncodePresetConfigEx failed: {}", (int)status);
        funcs_.nvEncDestroyEncoder(encoder_);
        encoder_ = nullptr;
        return false;
    }
    ...
    const uint32_t live_vbv_bits = (std::max)(bitrate / (std::max)(fps, 1u), 64'000u);
    encode_config_.rcParams.vbvBufferSize = live_vbv_bits;
    encode_config_.rcParams.vbvInitialDelay = live_vbv_bits;
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("InitializeEncoder failed: {}", (int)status);
        funcs_.nvEncDestroyEncoder(encoder_);
        encoder_ = nullptr;
        return false;
    }
    ...
    if (FAILED(hr)) {
        native_log_error("CreateTexture2D staging failed: {:#010x}", hr);
        funcs_.nvEncDestroyEncoder(encoder_);
        encoder_ = nullptr;
        return false;
    }
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("RegisterResource failed: {}", (int)status);
        funcs_.nvEncDestroyEncoder(encoder_);
        encoder_ = nullptr;
        return false;
    }
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("CreateBitstreamBuffer failed: {}", (int)status);
        funcs_.nvEncUnregisterResource(encoder_, registered_resource_);
        funcs_.nvEncDestroyEncoder(encoder_);
        encoder_ = nullptr;
        return false;
...
bool NvencEncoder::do_encode(NV_ENC_REGISTERED_PTR resource, int64_t timestamp_100ns) {
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("MapInputResource failed: {}", (int)status);
        return false;
    }
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("EncodePicture failed: {}", (int)status);
        return false;
    }
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("LockBitstream failed: {}", (int)status);
        return false;
    }
...
int NvencEncoder::register_input(ID3D11Texture2D* texture) {
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("RegisterResource (external {}) failed: {}",
...
void NvencEncoder::set_bitrate(uint32_t bitrate) {
    ...
    encode_config_.rcParams.averageBitRate = bitrate;
    encode_config_.rcParams.maxBitRate = bitrate;
    const uint32_t live_vbv_bits = (std::max)(bitrate / (std::max)(fps_, 1u), 64'000u);
    encode_config_.rcParams.vbvBufferSize = live_vbv_bits;
    encode_config_.rcParams.vbvInitialDelay = live_vbv_bits;
    ...
    if (status != NV_ENC_SUCCESS) {
        native_log_error("ReconfigureEncoder failed: {}", (int)status);
    }
}

EncoderInfo NvencEncoder::info() const {
    return {VideoBackend::NVENC, codec_, width_, height_};
}
